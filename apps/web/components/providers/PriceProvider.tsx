import { MarketTokensIds } from '@/lib/types/limitless';
import { useQuery } from '@tanstack/react-query';
import React, { createContext, useCallback, useContext } from 'react';

type CoingeckoPriceEntity = { usd: number };
export type GetCoingeckoPricesResponse = Record<
  MarketTokensIds,
  CoingeckoPriceEntity
>;
...
export const PriceOracleProvider = ({ children }: React.PropsWithChildren) => {
  ...
  const fetchTokenPrices = async () => {
    ...
    const data = (await response.json()) as GetCoingeckoPricesResponse;
