...
export const BalanceServiceProvider = ({ children }: PropsWithChildren) => {
  ...
  const whitelist = () => {};
  const etherspot = false;

  const walletAddress = useAccount().address;

  // const { mintErc20, transferErc20, unwrapEth, transferEthers, wrapEth } =
  //   useWeb3Service();
   ...
   * Weth balance
   */
  const { data: balanceOfSmartWallet, refetch: refetchbalanceOfSmartWallet } =
    useQuery({
      ...
      queryFn: async () => {
        ...
        //@ts-ignore
        const contractCallContext: ContractCallContext[] = supportedTokens?.map(
          (token) => ({
            reference: token.address,
            contractAddress: token.address,
            abi:
              token.priceOracleId === MarketTokensIds.WETH ? wethABI : erc20Abi,
            calls: [
              {
                reference: 'balance',
        ...
        try {
          ...
          balanceResult = supportedTokens?.map((token) => {
            return {
              ...
              decimals: token.decimals,
              value: BigInt(0),
              formatted: formatUnits(BigInt(0), token.decimals),
              image: token.logoUrl,
              contractAddress: token.address,
              price: marketTokensPrices
                ? marketTokensPrices[token.priceOracleId].usd
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      if (Number(ethFormatted) > gasFee) {
        ...
        //   eth - parseEther(gasFee.toString())
        // );
