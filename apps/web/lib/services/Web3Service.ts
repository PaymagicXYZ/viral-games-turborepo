...
import { useExternalWalletService } from './ExternalWalletService';
...
export function useWeb3Service(): Web3Service {
  ...
  const mintErc20 = async (
    ...
    // }
    return externalWalletService.mintErc20(
      token,
      value,
      smartWalletAddress,
      newToken,
  ...
  const checkAllowanceForAll = async (
    contractAddress: Address,
    spender: Address,
  ...
  const approveContract = async (
    ...
    externalWalletService.approveContractEOA(
      contractAddress,
      spender,
      BigInt(value),
    );

  const approveAllowanceForAll = async (
    contractAddress: Address,
    spender: Address,
